...
import { Code, Zap, DollarSign, Clock, TrendingDown, ArrowRight, Wrench } from 'lucide-react';
...
export default function DevelopmentVsAgentsPilotCalculator() {
  ...
  // Simple AgentsPilot pricing (without API)
  const BASE_MONTHLY_COST = 10; // Minimum monthly cost
  const COST_PER_AGENT = 5; // Additional cost per agent
  const COST_PER_PLUGIN = 2; // Additional cost per plugin
  ...
  const calculateComparison = () => {
    ...
    // AgentsPilot Costs (simple calculation)
    const monthlySubscription = BASE_MONTHLY_COST +
                               (numAgents * COST_PER_AGENT) +
                               (numAgents * avgPluginsPerAgent * COST_PER_PLUGIN);

    const agentsPilotFirstYear = monthlySubscription * 12;
    const agentsPilotThreeYear = monthlySubscription * 36;
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  onChange={(e) => updateInput('numAgents', parseInt(e.target.value) || 1)}
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  onChange={(e) => updateInput('avgPluginsPerAgent', parseInt(e.target.value) || 1)}
                  className="w-20 px-3 py-2 bg-zinc-800 border border-zinc-700 text-right font-bold text-white text-base focus:outline-none focus:border-orange-500 transition-colors"
                  min="1"
                />
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-start gap-3 pb-4 border-b border-zinc-800">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-2xl font-bold text-white">{result.agentsPilot.setupTime} {result.agentsPilot.setupTime === 1 ? 'day' : 'days'}</div>
                  </div>
                </div>
